...
export default function App() {
    ...
    const [extractedDenialData, setExtractedDenialData] =
        useState<DenialParsedData | null>(null);
    ...
    useEffect(() => {
        console.log(
            "App mounted, checking URL params:",
    ...
    const handleStartNewAppealWithPlan = async (
        planId: string,
        coveredPersonId: string
    ) => {
    ...
    const handlePlanPolicyUploadComplete = async (
        policyType: "comprehensive" | "supplementary",
        files: File[]
    ) => {
        ...
        const extractedData: InsurancePlanParsedData = {
            ...
            planName: "Connect Flex Bronze 0 NA/AN Under 300",
            policyNumber: "COINDEP0052023",
        };
        ...
        try {
            ...
            if (isSupabaseConfigured) {
                ...
                const uploadedFiles = await Promise.all(
                    planDraft.policyFiles.map(async (file) => {
                ...
                formData.append(
                    "userId",
                    user?._id || "691e93c4fd7adcd73e6f628c"
                );
                formData.append(
                    "insuranceCompany",
                    planDraft.planData.insuranceCompany || ""
                );
                formData.append("planName", planDraft.planData.planName || "");
                formData.append(
                    "policyNumber",
                    planDraft.planData.policyNumber || ""
                );
                ...
                formData.append(
                    "coveredIndividuals",
                    JSON.stringify(planDraft.coveredIndividuals || [])
                );
    ...
    const handleSaveEditedPlan = async (updatedPlan: InsurancePlan) => {
        try {
            ...
            const response = await fetch(
                apiUrl(`/api/plans/${updatedPlan.id}`),
                {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(updates),
            ...
            if (response.ok) {
                ...
                setInsurancePlans(
                    insurancePlans.map((p) =>
                        p.id === updatedPlan.id
                            ? { ...savedPlan, policyFiles: finalPolicyFiles }
                            : p
                    )
                );
    ...
    const handleDenialUploadComplete = async (
        files: (
            | File
            | {
                  name: string;
        ...
        try {
            ...
            const existingFiles = files.filter((f) => !(f instanceof File)) as {
                name: string;
                size: number;
                type: string;
                bucket?: string;
            ...
            if (isSupabaseConfigured) {
                ...
                if (newFiles.length > 0) {
                    ...
                    const response = await fetch(
                        apiUrl(`/api/cases/${currentCaseId}/files`),
                        {
                            method: "POST",
                            body: formData,
                        }
                    ...
                    if (!updatedCase) {
                        const response = await fetch(
                            apiUrl(`/api/cases/${currentCaseId}`)
                        );
            ...
            if (updatedCase) {
                setCases(
                    cases.map((c) => (c.id === currentCaseId ? updatedCase : c))
                );

                // Use hardcoded data instead of calling RAG pipeline
                const extracted: DenialParsedData = {
                    briefDescription:
    ...
    const handleDraftFollowup = async () => {
        ...
        try {
            const response = await fetch(
                apiUrl(`/api/cases/${currentCaseId}/generate-followup`),
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                }
    ...
    const handleSyncComplete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const hasReply =
                    updatedCase.emailThread &&
                    updatedCase.emailThread.some(
                        (e: any) => e.type === "received"
                    );
                if (hasReply) {
    ...
    const handleDeletePlan = async (planId: string) => {
        try {
            ...
            if (response.ok) {
                setInsurancePlans(
                    insurancePlans.filter((p) => p.id !== planId)
                );
            } else {
    ...
    const handleResolveCase = async (caseId: string, feedback?: string) => {
        const updates = {
            resolved: true,
    ...
    const handleDraftFollowupGenerated = (draft: {
        subject: string;
        body: string;
    }) => {
        if (!currentCaseId) return;
    ...
    const renderScreen = () => {
        ...
        switch (currentScreen) {
            ...
            case "edit-insurance-plan":
                const planToEdit = insurancePlans.find(
                        ...
                        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-gray-700 text-lg">
                                    Analyzing denial documents with AI...
                                </p>
