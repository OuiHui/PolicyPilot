      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <Card className="p-6 bg-gray-50">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900 font-medium">{planData.insuranceCompany}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Plan Name:</span>
                <span className="text-gray-900 font-medium">{planData.planName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Policy Number:</span>
                <span className="text-gray-900 font-medium">{planData.policyNumber}</span>
              </div>
              {planData.groupNumber && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Group Number:</span>
                  <span className="text-gray-900 font-medium">{planData.groupNumber}</span>
                </div>
              )}
            </div>
          </Card>
          ...
          <Card className="p-6 bg-gray-50">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900 font-medium capitalize">{policyType}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Files Uploaded:</span>
                <span className="text-gray-900 font-medium">{policyFiles.length}</span>
              </div>
            </div>
          ...
          <Card className="p-6 bg-gray-50">
            ...
            <div className="space-y-2">
              {coveredIndividuals.map((person) => (
                <div key={person.id} className="flex justify-between text-gray-600">
                  <span>{person.name}</span>
                  <span className="text-gray-900 font-medium">{person.relationship}</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="p-4 bg-green-50 border border-green-200 rounded-lg my-8">
          <p className="text-green-900">
            {fromAppealFlow ? (
              <>
                <strong>Next step:</strong> After saving this plan, you'll continue to create your appeal.
              </>
            ) : (
              <>
                <strong>Next step:</strong> Your insurance plan will be saved and you can use it to create appeals.
              </>
            )}
          </p>
        </div>
