...
type PolicyUploadProps = {
  initialPolicyType?: "comprehensive" | "supplementary" | null;
  initialFiles?: File[];
  onComplete: (
...
export function PolicyUpload({
  ...
  const [policyType, setPolicyType] = useState<
    "comprehensive" | "supplementary" | null
  >(initialPolicyType);
  const [comprehensiveFile, setComprehensiveFile] = useState<File | null>(
    initialPolicyType === "comprehensive" && initialFiles.length > 0
      ? initialFiles[0]
      : null
  );
  const [sobFiles, setSobFiles] = useState<File[]>(
    initialPolicyType === "supplementary"
      ? initialFiles.filter((f) => f.name.toLowerCase().includes("sob"))
      : []
  );
  const [eobFiles, setEobFiles] = useState<File[]>(
    initialPolicyType === "supplementary"
      ? initialFiles.filter((f) => f.name.toLowerCase().includes("eob"))
      : []
  );
  const [otherFiles, setOtherFiles] = useState<File[]>(
    initialPolicyType === "supplementary"
      ? initialFiles.filter(
          (f) =>
            !f.name.toLowerCase().includes("sob") &&
            !f.name.toLowerCase().includes("eob")
        )
      : []
  );
  ...
  const handleFileSelect = (
    ...
    if (multiple) {
      ...
      (setter as React.Dispatch<React.SetStateAction<File[]>>)((prev) => [
        ...prev,
        ...pdfFiles,
      ]);
    } else {
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <Card className="p-6 mb-6">
          ...
          <RadioGroup
            ...
            onValueChange={(value: string) =>
              setPolicyType(value as "comprehensive" | "supplementary")
            }
          >
            <div className="space-y-4">
              <div className="flex items-start space-x-3 p-4 border-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                ...
                >
                  ...
                  <span className="text-gray-600">
                    The complete, detailed policy from your insurer (also called
                    the Plan Document or Certificate of Coverage).
                  </span>
                </Label>
              </div>
              ...
              <div className="flex items-start space-x-3 p-4 border-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                ...
                >
                  ...
                  <span className="text-gray-600">
                    Such as Summary of Benefits (SOB) and Explanation of
                    Benefits (EOB)
                  </span>
                </Label>
              </div>
                ...
                {comprehensiveFile ? (
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500">
                        {(comprehensiveFile.size / 1024).toFixed(2)} KB
                      </p>
                    </div>
                    <button
                      onClick={(e) => {
                  ...
                  <>
                    ...
                    <p className="text-gray-900 mb-1">
                      Upload comprehensive policy document
                    </p>
                ...
                <div className="mt-4 space-y-2">
                  {sobFiles.map((file, index) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setSobFiles(sobFiles.filter((_, i) => i !== index))
                        }
                      >
                ...
                <div className="mt-4 space-y-2">
                  {eobFiles.map((file, index) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setEobFiles(eobFiles.filter((_, i) => i !== index))
                        }
                      >
                        <X className="w-4 h-4 text-gray-500" />
                      </button>
            ...
            <Card className="p-6">
              ...
              <label className="block">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors cursor-pointer">
                  ...
                  <p className="text-gray-500">
                    Multiple PDFs accepted - Optional
                  </p>
                </div>
              </label>
              {otherFiles.length > 0 && (
        ...
        <div className="flex items-start gap-3 p-4 bg-green-50 border border-green-200 rounded-lg mb-6">
          ...
          <p className="text-green-900">
            All files are encrypted and HIPAA compliant. Your documents are
            processed securely.
          </p>
        </div>
