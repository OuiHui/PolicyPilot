...
- [Simple upload](https://developers.google.com/workspace/gmail/api/guides/uploads#simple): `uploadType=media`. For quick transfer of smaller files, for example, 5 MB or less.
- [Multipart upload](https://developers.google.com/workspace/gmail/api/guides/uploads#multipart): `uploadType=multipart`. For quick transfer of smaller files and metadata; transfers the file along with metadata that describes it, all in a single request.
- [Resumable upload](https://developers.google.com/workspace/gmail/api/guides/uploads#resumable): `uploadType=resumable`. For reliable transfer, especially important with larger files. With this method, you use a session initiating request, which optionally can include metadata. This is a good strategy to use for most applications, since it also works for smaller files at the cost of one additional HTTP request per upload.
...
- **The /upload URI, for the media.** The format of the upload endpoint is the
  ...
  Example: `POST /upload/gmail/v1/users/`<var class="apiparam" translate="no">userId</var>`/messages/send`
- **The standard resource URI, for the metadata.** If the resource contains any
  ...
  Example:
  `POST /gmail/v1/users/`<var class="apiparam" translate="no">userId</var>`/messages/send`
...
- `Content-Type`. Set to one of the method's accepted upload media data types, specified in the API [](https://developers.google.com/gmail/api/reference/rest)reference.
...
`https://www.googleapis.com/gmail/v1/users/`<var class="apiparam" translate="no">userId</var>`/messages/send`
...
1. [Start a resumable session](https://developers.google.com/workspace/gmail/api/guides/uploads#start-resumable). Make an initial request to the upload URI that includes the metadata, if any.
2. [Save the resumable session URI](https://developers.google.com/workspace/gmail/api/guides/uploads#save-session-uri). Save the session URI returned in the response of the initial request; you'll use it for the remaining requests in this session.
3. [Upload the file](https://developers.google.com/workspace/gmail/api/guides/uploads#upload-resumable). Send the media file to the resumable session URI.

In addition, apps that use resumable upload need to have code to [resume an interrupted upload](https://developers.google.com/workspace/gmail/api/guides/uploads#resume-upload). If an upload is interrupted, find out how much data was successfully received, and then resume the upload starting from that point.
...
If the upload request is interrupted or if you receive an `HTTP 503 Service Unavailable` or any other `5xx` response from the server, follow the procedure outlined in [resume an interrupted upload](https://developers.google.com/workspace/gmail/api/guides/uploads#resume-upload).
...
If any chunk's `PUT` request is interrupted or if you receive an HTTP `503 Service Unavailable` or any other `5xx` response from the server, follow the procedure outlined in [resume an interrupted upload](https://developers.google.com/workspace/gmail/api/guides/uploads#resume-upload), but instead of uploading the rest of the file, simply continue uploading chunks from that point.
...
- Resume or retry uploads that fail due to connection interruptions or any `5xx` errors, including:
  ...
  - `504 Gateway Timeout`
- Use an [exponential backoff](https://developers.google.com/workspace/gmail/api/guides/uploads#exp-backoff) strategy if any `5xx` server error is returned when resuming or retrying upload requests. These errors can occur if a server is getting overloaded. Exponential backoff can help alleviate these kinds of problems during periods of high volume of requests or heavy network traffic.
